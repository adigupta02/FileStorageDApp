...
const client = new Web3Storage({
	token:
		"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweDFiMmUxMzhhQUJDODQwMjYwQkNjNzFlZUI1YTlCOUFlRjJGNTRBQ0YiLCJpc3MiOiJ3ZWIzLXN0b3JhZ2UiLCJpYXQiOjE2ODkxNTcyNjA0MTYsIm5hbWUiOiJGaWxlU3RvcmFnZSJ9.luciJXFkLn8iEe-MQWa_3H5itmFcYUIDK_D28WM9NMc",
});
		...
		if (file) {
			try {
				...
				contract.on("FileUploaded", (fileId, fileHash, fileSize, fileType, fileName, uploadTime, uploader) => {
					...
					const istUploadTime = new Date(Number(uploadTime) * 1000).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })
					const file_obj = {
	...
	const getFilesUploaded = async () => {
		try {
			...
			for (let i = 0; i < +fileCount?.toString(); i++) {
				...
				const istUploadTime = new Date(Number(file[5]) * 1000).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })
				const file_obj = {
					id: file[0]?.toString(),
					hash: file[1],
